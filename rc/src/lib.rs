use std::{cell::Cell, fmt::Debug, ops::Deref};

pub struct MyRc<T> {
    inner: *const MyInner<T>,
    _marker: std::marker::PhantomData<MyInner<T>>,
}

impl<T: Debug> Debug for MyRc<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "MyRc ({:?})", unsafe { &*self.inner })
    }
}
...
impl<T> MyRc<T> {
    pub fn new(value: T) -> Self {
        Self {
            inner: Box::into_raw(Box::new(MyInner { inner: value, references: Cell::new(1) })),
...
impl<T> Drop for MyRc<T> {
    fn drop(&mut self) {
        ...
        if inner.references.get() == 0 {
            ...
            let _ = unsafe { Box::from_raw(self.inner as *mut MyInner<T>) };
        }
